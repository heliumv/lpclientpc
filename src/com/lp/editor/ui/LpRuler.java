...
// Some defines used in painting the ruler.
  ...
  public LpRuler(LpDecoratedTextPane text) {
    this();
    setTextPane(text);
  }
   ...
   * @param text ein LpTextPane.
   */
  public void setTextPane(LpDecoratedTextPane text) {
    if (textPane != null) {
   ...
   *
   * @return Das assoziierte TextPane.
   */
  ...
  void updateRuler() {
    ...
    if (text != null) {
      Element pe = text.getStyledDocument().getParagraphElement(0);
  ...
  protected void paintUnits(Graphics g, Rectangle clip, Insets insets) {
    ...
    for (int x = Math.max(xOffset, clip.x / DPI * DPI + dpiOffset),
         maxX = clip.x + clip.width; x <= maxX; x += E_DPI) {
  ...
  protected void paintTabs(Graphics g, Rectangle clip, Insets insets) {
    ...
    if (tabs == null) {
      ...
      do {
        tab = tabs.getTabAfter( (float) lastX + .01f);
        if (tab != null) {
  ...
  public Dimension getPreferredSize() {
    ...
    return new Dimension(textPane.getPreferredSize().width, getUnitsFontHeight());
  }
