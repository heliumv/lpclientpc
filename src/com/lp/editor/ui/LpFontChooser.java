...
JTextPane preview;
   ...
   * @param mode int
   * @param fontNames Verfuegbare Schriftarten.
   * @param fontSizes Vordefinierte Schriftgroessen.
   * @param locale Locale
   */
  public LpFontChooser(JFrame parent, int mode, String[] fontNames,
                       String[] fontSizes, Locale locale) {
    super(parent,
          LpEditorMessages.getInstance().getString("FontChooser.ChangeTextProp"), true);
    ...
    underlineCheck = new JCheckBox(LpEditorMessages.getInstance().getString(
        "FontChooser.Underline"));
    underlineCheck.setActionCommand("underline");
    p.add(underlineCheck);

    strikethroughCheck = new JCheckBox(LpEditorMessages.getInstance().getString(
        "FontChooser.Strikethrough"));
    strikethroughCheck.setActionCommand("strikethrough");
    p.add(strikethroughCheck);

    subscriptCheck = new JCheckBox(LpEditorMessages.getInstance().getString(
        "FontChooser.Subscript"));
    subscriptCheck.setActionCommand("subscript");

    superscriptCheck = new JCheckBox(LpEditorMessages.getInstance().getString(
        "FontChooser.Superscript"));
    ...
    // ****** siehe http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4694008
    ...
    JButton okButton = new JButton(LpEditorMessages.getInstance().getString(
        "Ok"));
    ...
    JButton cancelButton = new JButton(LpEditorMessages.getInstance().getString(
        "Cancel"));
  ...
  public void actionPerformed(ActionEvent ae) {
    if (ae.getActionCommand().equals("fgcolor")) {
      ...
      Color oldColor;
      if (preview.getCharacterAttributes().isDefined(StyleConstants.Background)) {
        oldColor = StyleConstants.getBackground(preview.getCharacterAttributes());
      }
      else {
        oldColor = null;
      }
      Color newColor = colorChooser.showDialog(this,
                                               LpEditorMessages.getInstance().
                                               getString(
   ...
   *
   * @param attr Die zu bearbeitenden Attribute.
   * @param text Der selektierte Text. Dieser wird im preview-Fenster
  ...
  private void updatePreview() {
    ...
    StyleConstants.setFontSize(styleAttributes,
                               Integer.parseInt( (String) fontSizeCombo.
                                                getSelectedItem()));
    ...
    StyleConstants.setUnderline(styleAttributes, underlineCheck.isSelected());
    StyleConstants.setStrikeThrough(styleAttributes,
                                    strikethroughCheck.isSelected());
    if (mode != LpEditor.MODE_JASPER) {
      StyleConstants.setSubscript(styleAttributes, subscriptCheck.isSelected());
      StyleConstants.setSuperscript(styleAttributes,
                                    superscriptCheck.isSelected());
    }
    //StyleConstants.setForeground(styleAttributes, fgColorLabel.getBackground());
    StyleConstants.setForeground(styleAttributes, fgColorButton.getBackground());
    if (bOpaqueBackground) {
      //StyleConstants.setBackground(styleAttributes, bgColorLabel.getBackground());
      StyleConstants.setBackground(styleAttributes, bgColorButton.getBackground());
