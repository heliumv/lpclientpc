...
/**
 * <p>In diesem Fenster werden Parameter fuer den Anwender erfasst.
 ...
 */
public class PanelParameterAnwender extends PanelBasis {
	/**
	 * 
	 */
	...
	private ParameteranwenderDto parameteranwenderDto = null;
	...
	public PanelParameterAnwender(InternalFrame internalFrame,
	...
	public void eventActionSave(ActionEvent e, boolean bNeedNoSaveI)
		...
		if (allMandatoryFieldsSetDlg()) {
			if (Helper.istWertVomTyp(wtfWert.getText(), wtfDatentyp.getText())) {
				...
				if (parameteranwenderDto.getCNr() == null) {
					ParameteranwenderPK pkParameteranwender = DelegateFactory
							.getInstance().getParameterDelegate()
							.createParameteranwender(parameteranwenderDto);
					parameteranwenderDto = DelegateFactory
							.getInstance()
							.getParameterDelegate()
							.parameteranwenderFindByPrimaryKey(
									pkParameteranwender);
					setKeyWhenDetailPanel(pkParameteranwender);
				} else {
					DelegateFactory.getInstance().getParameterDelegate()
							.updateParameteranwender(parameteranwenderDto);
				}
	...
	private void resetPanel() throws Throwable {
		parameteranwenderDto = new ParameteranwenderDto();
		leereAlleFelder(this);
	}

	private void dto2Components() throws Throwable {
		wtfBezeichnung.setText(parameteranwenderDto.getCNr());
		wtfKategorie.setText(parameteranwenderDto.getCKategorie());
		wtfWert.setText(parameteranwenderDto.getCWert());
		wtfBemerkungsmall.setText(parameteranwenderDto.getCBemerkungsmall());
		wefBemerkunglarge.setText(parameteranwenderDto.getCBemerkunglarge());
		wtfDatentyp.setText(parameteranwenderDto.getCDatentyp());
	}

	private void components2Dto() throws Throwable {
		// es werden keine neuen Parameter angelegt
		parameteranwenderDto.setCWert(wtfWert.getText());
		parameteranwenderDto.setCBemerkungsmall(wtfBemerkungsmall.getText());
		parameteranwenderDto.setCBemerkunglarge(wefBemerkunglarge.getText());
		parameteranwenderDto.setCDatentyp(wtfDatentyp.getText());
	}
	...
	public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI)
		...
		if (oKey != null && !oKey.equals(LPMain.getLockMeForNew())) {
			...
			pkParameteranwender.setCNr(((FLRParameteranwenderPK) oKey)
					.getC_nr());
			pkParameteranwender.setCKategorie(((FLRParameteranwenderPK) oKey)
					.getC_kategorie());

			parameteranwenderDto = DelegateFactory.getInstance()
					.getParameterDelegate()
					.parameteranwenderFindByPrimaryKey(pkParameteranwender);
			dto2Components();
		}
	...
	private void aktualisiereStatusbar() throws Throwable {
		if (parameteranwenderDto != null
				&& parameteranwenderDto.getCNr() != null) {
			setStatusbarPersonalIIdAendern(parameteranwenderDto
					.getPersonalIIdAendern());
			setStatusbarTAendern(parameteranwenderDto.getTAendern());
		}
	}

	protected String getLockMeWer() throws Exception {
		return HelperClient.LOCKME_PARAMETERANWENDER;
	}
}
