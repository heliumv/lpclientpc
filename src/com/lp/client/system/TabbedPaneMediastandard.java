 ...
 */
public class TabbedPaneMediastandard extends TabbedPane {
	...
	private PanelSplit panelSplitReportvariante = null;
	private PanelReportvariante panelBottomReportvariante = null;
	private PanelQuery panelQueryReportvariante = null;

	// Reihenfolge der Panels
	private static final int IDX_PANEL_MEDIA = 0;
	private static final int IDX_PANEL_MEDIAART = 1;
	private static final int IDX_PANEL_REPORTVARIANTE = 2;
	...
	public void lPEventItemChanged(ItemChangedEvent eI) throws Throwable {
		...
		if (e.getID() == ItemChangedEvent.ITEM_CHANGED) {
			...
			} else if (e.getSource() == panelQueryReportvariante) {
				Object key = ((ISourceEvent) e.getSource()).getIdSelected();
				panelBottomReportvariante.setKeyWhenDetailPanel(key);
				panelBottomReportvariante.eventYouAreSelected(false);
				panelQueryReportvariante.updateButtons();

			}

		} else if (e.getID() == ItemChangedEvent.ACTION_UPDATE) {
			if (e.getSource() == panelMedia) {
				panelQueryMedia.updateButtons(new LockStateValue(
						PanelBasis.LOCK_FOR_NEW));
				;
			} else if (e.getSource() == panelBottomMediaart) {
				panelQueryMediaart.updateButtons(new LockStateValue(
						PanelBasis.LOCK_FOR_NEW));
				;
			} else if (e.getSource() == panelBottomReportvariante) {
				panelQueryReportvariante.updateButtons(new LockStateValue(
						PanelBasis.LOCK_FOR_NEW));
				;
			}
		}
			...
			if (e.getSource() == panelMedia) {
				...
				panelSplitReportvariante.eventYouAreSelected(false);
			}
		...
		// selektiert nach save: 0 Wir landen hier nach Button Save
			...
			if (e.getSource() == panelMedia) {
				...
				Object oKey = panelBottomReportvariante.getKeyWhenDetailPanel();
				panelQueryReportvariante.eventYouAreSelected(false);
				panelQueryReportvariante.setSelectedId(oKey);
				panelSplitReportvariante.eventYouAreSelected(false);
			}
		}
		...
		// wir landen hier nach der Abfolge Button Neu -> xx -> Button Discard
			...
			if (e.getSource() == panelMedia) {
				...
				getInternalFrame().setKeyWasForLockMe(
						oKey == null ? null : oKey + "");
				panelSplitMedia.eventYouAreSelected(false); // refresh auf das
															// gesamte 1:n panel
			} else if (e.getSource() == panelBottomMediaart) {
				panelSplitMediaart.eventYouAreSelected(false);
			} else if (e.getSource() == panelBottomReportvariante) {
				Object oKey = panelQueryReportvariante.getSelectedId();
				if (oKey != null) {
					getInternalFrame().setKeyWasForLockMe(oKey.toString());
				} else {
					getInternalFrame().setKeyWasForLockMe(null);
				}
				if (panelBottomReportvariante.getKeyWhenDetailPanel() == null) {
					Object oNaechster = panelQueryReportvariante
							.getId2SelectAfterDelete();
					panelQueryReportvariante.setSelectedId(oNaechster);
				}
				panelSplitReportvariante.eventYouAreSelected(false);
			}
		}
			...
			if (e.getSource() == panelQueryMedia) {
				...
				panelBottomReportvariante.eventActionNew(e, true, false);
				panelBottomReportvariante.eventYouAreSelected(false);
			}
		}
	}
		...
		case IDX_PANEL_MEDIAART:
			...
			if (panelQueryMediaart.getSelectedId() == null) {
				getInternalFrame().enableAllOberePanelsExceptMe(this,
						IDX_PANEL_MEDIAART, false);
			}
			break;

		case IDX_PANEL_REPORTVARIANTE:
			refreshPanelReportvariante();
			panelQueryReportvariante.eventYouAreSelected(false);
	...
	private PanelSplit getPanelSplitMedia(boolean bNeedInstantiationIfNull)
		...
		if (panelSplitMedia == null && bNeedInstantiationIfNull) {
			...
			panelSplitMedia = new PanelSplit(getInternalFrame(),
					getPanelMedia(true), getPanelQueryMedia(true), 200);
	...
	private void refreshPanelMediaart() throws Throwable {
		...
		if (panelSplitMediaart == null) {
			...
			panelSplitMediaart = new PanelSplit(getInternalFrame(),
					panelBottomMediaart, panelQueryMediaart,
	...
	private void refreshPanelReportvariante() throws Throwable {

		if (panelSplitReportvariante == null) {
			...
			setComponentAt(IDX_PANEL_REPORTVARIANTE, panelSplitReportvariante);
		}
	}

	private void refreshPanelMediastandard() throws Throwable {
