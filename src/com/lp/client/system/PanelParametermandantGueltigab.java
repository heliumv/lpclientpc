 ...
 */
public class PanelParametermandantGueltigab extends PanelBasis {
	/**
	 * 
	 */
	...
	private ParametermandantgueltigabDto parametermandantgueltigabDto = null;
	...
	public PanelParametermandantGueltigab(InternalFrame internalFrame,
	...
	void jbInit() throws Throwable {
		...
		String[] aWhichButtonIUse = { PanelBasis.ACTION_UPDATE,
				PanelBasis.ACTION_SAVE, PanelBasis.ACTION_DELETE, PanelBasis.ACTION_DISCARD };
		...
		wtfBemerkungsmall.setActivatable(false);
		...
		// Zeile
		iZeile++;
		jPanelWorkingOn.add(wlaGueltigab, new GridBagConstraints(0, iZeile, 1,
				1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.BOTH,
				new Insets(2, 2, 2, 2), 0, 0));

		jPanelWorkingOn.add(wdfGueltigab, new GridBagConstraints(1, iZeile, 1,
				1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		iZeile++;
	...
	protected void eventActionDelete(ActionEvent e,
		...
		DelegateFactory.getInstance().getParameterDelegate()
				.removeParametermandantgueltigab(parametermandantgueltigabDto);
	...
	public void eventActionSave(ActionEvent e, boolean bNeedNoSaveI)
		...
		if (allMandatoryFieldsSetDlg()) {
			...
			if (Helper.istKennwortParameter(wtfBezeichnung.getText())) {
			...
			if (bSpeichern) {
				...
				ParametermandantgueltigabPK pk = new ParametermandantgueltigabPK(
						parametermandantgueltigabDto.getCNr(),
						parametermandantgueltigabDto.getMandantCNr(),
						parametermandantgueltigabDto.getCKategorie(),
						parametermandantgueltigabDto.getTGueltigab());

				DelegateFactory
						.getInstance()
						.getParameterDelegate()
						.updateParametermandantgueltigab(
								parametermandantgueltigabDto);

				setKeyWhenDetailPanel(pk);
	...
	private void resetPanel() throws Throwable {
		parametermandantgueltigabDto = new ParametermandantgueltigabDto();
		leereAlleFelder(this);
	}

	private void dto2Components() throws Throwable {

		wtfWert.setText(parametermandantgueltigabDto.getCWert());
		wdfGueltigab.setTimestamp(parametermandantgueltigabDto.getTGueltigab());
	...
	private void components2Dto() throws Throwable {
		...
		parametermandantgueltigabDto.setTGueltigab(wdfGueltigab.getTimestamp());
		parametermandantgueltigabDto.setCNr(wtfBezeichnung.getText());
		parametermandantgueltigabDto.setCKategorie(wtfKategorie.getText());
		parametermandantgueltigabDto.setMandantCNr(LPMain.getTheClient()
				.getMandant());

		if (Helper.istKennwortParameter(wtfBezeichnung.getText())) {
			parametermandantgueltigabDto.setCWert(new String(wtfPassword
					.getPassword()));
		} else {
			parametermandantgueltigabDto.setCWert(wtfWert.getText());
		}
	...
	public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI)
		...
		intFrame.getTabbedPaneParameter().getPanelQueryParametermandant();
		FLRParametermandantPK pk = (FLRParametermandantPK) intFrame
				.getTabbedPaneParameter().getPanelQueryParametermandant()
				.getSelectedId();

		ParametermandantPK pkParametermandant = new ParametermandantPK();
		pkParametermandant.setCNr(pk.getC_nr());
		pkParametermandant.setMandantCNr(pk.getMandant_c_nr());
		pkParametermandant.setCKategorie(pk.getC_kategorie());

		ParametermandantDto pmDto = DelegateFactory.getInstance()
				.getParameterDelegate()
				.parametermandantFindByPrimaryKey(pkParametermandant);
		wtfDatentyp.setText(pmDto.getCDatentyp());

		wtfBemerkungsmall.setText(pmDto.getCBemerkungsmall());

		wtfBezeichnung.setText(pmDto.getCNr());
		wtfKategorie.setText(pmDto.getCKategorie());
		...
		if (oKey != null && !oKey.equals(LPMain.getLockMeForNew())) {
			...
			pkParametermandantGueltigab
					.setCNr(((FLRParametermandantgueltigabPK) oKey).getC_nr());
			pkParametermandantGueltigab
					.setMandantCNr(((FLRParametermandantgueltigabPK) oKey)
							.getMandant_c_nr());
			pkParametermandantGueltigab
					.setCKategorie(((FLRParametermandantgueltigabPK) oKey)
							.getC_kategorie());
			pkParametermandantGueltigab.setTGueltigab(new java.sql.Timestamp(
					((FLRParametermandantgueltigabPK) oKey).getT_gueltigab()
							.getTime()));

			parametermandantgueltigabDto = DelegateFactory
					.getInstance()
					.getParameterDelegate()
					.parametermandantgueltigabFindByPrimaryKey(
							pkParametermandantGueltigab);
			dto2Components();
		}
