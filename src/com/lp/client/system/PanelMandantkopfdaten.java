 ...
 */
public class PanelMandantkopfdaten
...
private WrapperLabel wlaMandant = null;
  ...
  protected void eventActionRefresh(ActionEvent aE, boolean bNeedNoRefreshI)
    ...
    this.checkMandantLoggedInEqualsMandantSelected(LPMain.getInstance().getTheClient().
        getMandant(), getMandantDto().getCNr());
  }
  ...
  public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI)
    ...
    checkMandantLoggedInEqualsMandantSelected(LPMain.getInstance().getTheClient().
                                              getMandant(), getMandantDto().getCNr());
  ...
  private void jbInit()
    ...
    wlaMandant = new WrapperLabel(LPMain.getInstance().
                                  getTextRespectUISPr("lp.mandant"));
    wtfMandant = new WrapperTextField(MandantFac.MAX_MANDANT);
    wtfMandant.setMandatoryFieldDB(true);

    wlaWaehrung.setText(LPMain.getInstance().
                        getTextRespectUISPr("lp.waehrung"));
  ...
  protected void components2Dto()
    ...
    String sKBez = getMandantDto().getCKbez() != null ?
        getMandantDto().getCKbez() : wtfMandant.getText();
    ...
    if (allMandatoryFieldsSetDlg()) {
      ...
      try {
        if (getMandantDto().getCNr() == null) {
          ...
          getMandantDto().setPartnerIIdLieferadresse(getMandantDto().getPartnerIId());
          try {
            String cNr = DelegateFactory.getInstance().getMandantDelegate().createMandant(
                getMandantDto());
            ...
            if ( ( (ExceptionLP) t).getICode() == EJBExceptionLP.FEHLER_DUPLICATE_UNIQUE) {
              DialogFactory.showModalDialog(LPMain.getInstance().
                  ...
                  LPMain.getInstance().getTextRespectUISPr("lp.mandantschonangelegt"));
            }
  ...
  protected void eventActionDelete(ActionEvent e,
    ...
    if (getLockedstateDetailMainKey().getIState() == LOCK_IS_NOT_LOCKED) {
      DelegateFactory.getInstance().getMandantDelegate().removeMandant(getMandantDto());
...
//Waehrung mit EUR default vorbesetzen
    tmWaehrungen = DelegateFactory.getInstance().getLocaleDelegate().getAllWaehrungen();
    wcbWaehrung.setMap(tmWaehrungen);

    Iterator<?> it = tmWaehrungen.keySet().iterator();
    int count = 0;
    while (it.hasNext()) {
      if (it.next().toString().equals(Currency.getInstance(Helper.string2Locale(LocaleFac.
          LP_LOCALE_DE_AT)).getCurrencyCode())) {
        break;
      }
      count++;
    }
...
//          LPMain.getInstance().getTheClient().getMandant());
    ...
    try {
      ...
      if (getLockedstateDetailMainKey().getIState() == LOCK_FOR_NEW
        ...
        if (getPartnerDto().getIId() == null // neu
            || (wtfName1.getText() != null &&
                !wtfName1.getText().equals(getPartnerDto().getCName1nachnamefirmazeile1())) // name1 geaendert
            || isBNeuAusPartner()) { // neu aus

          if (wtfName1.getText() != null && !wtfName1.getText().equals("")) {
            PartnerDto[] p =
            ...
            if (p.length > 1) {
              ...
              if ( (JOptionPane.showOptionDialog(null,
                                               LPMain.getInstance().
                                               getTextRespectUISPr("part.existiert_mehrere"),
                                               "",
              ...
              FilterKriterium[] filters = createFKPartnerName1(wtfName1.getText());
              ...
              panelQueryFLRPartner.befuellePanelFilterkriterienDirekt(
                  SystemFilterFactory.getInstance().createFKDLandPlzOrtOrt(),
                  null);
              // vorbesetzen
              if (getPartnerDto().getIId() != null) {
                panelQueryFLRPartner.setSelectedId(getPartnerDto().getIId());
              }
             ...
             //Existiert nur ein Partner mit demselben Namen
           {
             ...
             if ( (JOptionPane.showOptionDialog(null,
                                         LPMain.getInstance().
                                         getTextRespectUISPr("part.existiert_ein"),
                                         "",
                                         JOptionPane.YES_NO_OPTION,
                                         JOptionPane.QUESTION_MESSAGE,
                                         null, options, options[0])) ==
  ...
  /**
   * Default Filterkriterium fuer Filter nach Partner Name 1.
   * <br>Bedingung: Attributname im FLR ist
   * PartnerFac.FLR_PARTNER_C_NAME1NACHNAMEFIRMAZEILE1.
   *
   * @param sPartnerName1 Integer
   * @return FilterKriterium[]
   */
  public FilterKriterium[] createFKPartnerName1(String sPartnerName1) {
    FilterKriterium[] kriterien = new FilterKriterium[1];
    FilterKriterium krit1 = new FilterKriterium(PartnerFac.FLR_PARTNER_C_NAME1NACHNAMEFIRMAZEILE1,
                                                true,
                                                "'" + sPartnerName1 + "'",
      ...
      if (e.getSource() == panelQueryFLROrt) {
       ...
       if (keyLandPLZOrt != null) {
         LandplzortDto landPLZOrtDto = DelegateFactory.getInstance().
             getSystemDelegate().landplzortFindByPrimaryKey( (Integer) keyLandPLZOrt);
