 ...
 */
public class PanelAngebotstklPositionen
...
/** Cache for convenience. */
  ...
  public PanelAngebotstklPositionen(InternalFrame internalFrame, String add2TitleI,
                                    Object key)
      throws Throwable {
  ...
  private void initPanel()
    ...
    setPositionsarten(DelegateFactory.getInstance().getAngebotstklDelegate().
                      getAllAgstklpositionsart());
  }
  ...
  protected void setDefaults()
    ...
    if (intFrame.getAgstklDto() != null && intFrame.getAgstklDto().getIId() != null) {
      // alle auftragabhaengigen Defaults in den verschiedenen PanelPositionen setzen
      ...
      ( (PanelPositionenArtikelVerkauf) panelArtikel).setKundeDto(
          DelegateFactory.getInstance().getKundeDelegate().kundeFindByPrimaryKey(
              intFrame.getAgstklDto().getKundeIId()));
      ( (PanelPositionenArtikelVerkauf) panelArtikel).setWechselkurs(new Double(intFrame.
          getAgstklDto().getFWechselkursmandantwaehrungzuagstklwaehrung().doubleValue()));
      ( (PanelPositionenArtikelVerkauf) panelArtikel).setCNrWaehrung(intFrame.
          getAgstklDto().getWaehrungCNr());
      ( (PanelPositionenArtikelVerkauf) panelArtikel).
          setGueltigkeitsdatumArtikeleinzelverkaufspreis(Helper.extractDate(intFrame.
          getAgstklDto().getTBelegdatum()));
      ( (PanelPositionenArtikelVerkauf) panelArtikel).
          setGueltigkeitsdatumArtikeleinzelverkaufspreis(new java.sql.Date(System.
          currentTimeMillis()));
  ...
  public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI)
    ...
    // wenn das Angebot gerade von mir gelockt ist, die Artikeleingabefelder freischalten
  ...
  public void eventActionSave(ActionEvent e, boolean bNeedNoSaveI)
    ...
    try {
      ...
      if (allMandatoryFieldsSetDlg()) {
        ...
        if (bDiePositionSpeichern) {
          bDiePositionSpeichern = HelperClient.checkNumberFormat(panelArtikel.
              wnfBruttopreis.
              getBigDecimal());
        }

        if (bDiePositionSpeichern) {
          if (agstklpositionDto.getAgstklpositionsartCNr().equalsIgnoreCase(
            ...
            bDiePositionSpeichern = DialogFactory.pruefeUnterpreisigkeitDlg(
                ...
                intFrame.getAgstklDto().getFWechselkursmandantwaehrungzuagstklwaehrung().
                doubleValue(),
                agstklpositionDto.getNMenge());
            if (bDiePositionSpeichern) {
              // Soll eine Position mit einem eventuellen zugehoerigen Artikel angelegt werden?
              if (agstklpositionDto.getIId() == null) {
                bPositionFuerZugehoerigenArtikelAnlegen = DialogFactory.
                    pruefeZugehoerigenArtikelAnlegenDlg(panelArtikel.getArtikelDto());
              }
            }
          }
        }
        ...
        if (bDiePositionSpeichern) {
          if (agstklpositionDto.getIId() == null) {
            // Soll die neue Position vor der aktuell selektierten stehen?
            if (bFuegeNeuePositionVorDerSelektiertenEin) {
              ...
              // Die neue Position wird an frei gemachte Position gesetzt
    ...
    // wenn eine Position fuer einen zugehoerigen Artikel angelegt werden soll,
    ...
    if (bPositionFuerZugehoerigenArtikelAnlegen) {
      ...
      tpAngebotstkl.getAngebotstklPositionenTop().updateButtons(
          tpAngebotstkl.getAngebotstklPositionenBottom().getLockedstateDetailMainKey());
  ...
  protected void eventActionDelete(ActionEvent e, boolean bAdministrateLockKeyI,
      ...
      throws
      Throwable {
    DelegateFactory.getInstance().getAngebotstklpositionDelegate().removeAgstklposition(
        agstklpositionDto);
  ...
  private void dto2Components()
    ...
    if (positionsart.equalsIgnoreCase(AngebotstklServiceFac.AGSTKLPOSITIONART_IDENT)) {
      ...
      // die Artikelbezeichnung kann in der Angebotposition uebersteuert sein
      ...
      ( (PanelPositionenArtikelVerkauf) panelArtikel).verkaufspreisDtoInZielwaehrung = new
          VerkaufspreisDto();
      ( (PanelPositionenArtikelVerkauf) panelArtikel).verkaufspreisDtoInZielwaehrung.
          einzelpreis = agstklpositionDto.getNNettoeinzelpreis();

      BigDecimal nRabattsumme =
          agstklpositionDto.getNNettoeinzelpreis().multiply(
              new BigDecimal(agstklpositionDto.getFRabattsatz().doubleValue()).
              movePointLeft(2));

      ( (PanelPositionenArtikelVerkauf) panelArtikel).
          verkaufspreisDtoInZielwaehrung.rabattsumme = nRabattsumme;
      ( (PanelPositionenArtikelVerkauf) panelArtikel).
          verkaufspreisDtoInZielwaehrung.nettopreis = agstklpositionDto.
          getNNettogesamtpreis();
      ( (PanelPositionenArtikelVerkauf) panelArtikel).
          verkaufspreisDtoInZielwaehrung.mwstsumme = new BigDecimal(0);
      ( (PanelPositionenArtikelVerkauf) panelArtikel).
          verkaufspreisDtoInZielwaehrung.bruttopreis = new BigDecimal(0);
      ( (PanelPositionenArtikelVerkauf) panelArtikel).
          verkaufspreisDtoInZielwaehrung.rabattsatz = new Double(agstklpositionDto.
          getFRabattsatz().doubleValue());

      if (agstklpositionDto.getFZusatzrabattsatz() != null) {
        ( (PanelPositionenArtikelVerkauf) panelArtikel).
            verkaufspreisDtoInZielwaehrung.setDdZusatzrabattsatz(
                new Double(agstklpositionDto.getFZusatzrabattsatz().doubleValue()));

        BigDecimal nZusatzrabattsumme = agstklpositionDto.getNNettoeinzelpreis().subtract(
            nRabattsumme).multiply(
                new BigDecimal(agstklpositionDto.getFZusatzrabattsatz().doubleValue())).
            movePointLeft(2);

        ( (PanelPositionenArtikelVerkauf) panelArtikel).
            verkaufspreisDtoInZielwaehrung.setNZusatzrabattsumme(nZusatzrabattsumme);
      }

      ( (PanelPositionenArtikelVerkauf) panelArtikel).
          verkaufspreisDto2components();
    }
    else if (positionsart.equalsIgnoreCase(AngebotstklServiceFac.
                                           AGSTKLPOSITIONART_HANDEINGABE)) {
      ...
      panelHandeingabe.getWnfRabattsatz().setDouble(new Double(agstklpositionDto.
          getFRabattsatz().doubleValue()));
      ...
      if (agstklpositionDto.getFZusatzrabattsatz() != null) {
        panelHandeingabe.wnfZusatzrabattsatz.setDouble(new Double(agstklpositionDto.
  ...
  private void components2Dto()
    ...
    if (positionsart.equalsIgnoreCase(AngebotstklServiceFac.AGSTKLPOSITIONART_IDENT)) {
      ...
      if ( ( (PanelPositionenArtikelVerkauf) panelArtikel).
          bIstRabattsatzDefaultUebersteuert) {
      ...
      agstklpositionDto.setBRabattsatzuebersteuert(new Short( (short) 0));
      agstklpositionDto.setNNettoeinzelpreis(panelHandeingabe.wnfEinzelpreis.
                                             getBigDecimal());
      agstklpositionDto.setFZusatzrabattsatz(panelHandeingabe.getWnfZusatzrabattsatz().
                                             getDouble());
