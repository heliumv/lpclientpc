 ...
 */
public class PanelTabelleRechnungUmsatz
...
private WrapperLabel wlaKritAuswertung = null;
   ...
   * @throws Throwable
   */
  public PanelTabelleRechnungUmsatz(
      int iUsecaseIdI,
  ...
  private void jbInit() throws Exception {
    ...
    wlaUmsatz = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr("lp.umsatzuebersicht"));
    ...
    getPanelOptionaleZweiteZeile().add(wlaUmsatz,
        ...
        GridBagConstraints.BOTH,
        new Insets(2, 2, 2, 2), 0, 0));
    getPanelOptionaleZweiteZeile().add(wlaAnzahlung,
    ...
    TableColumn tcEmpty = getTable().getColumnModel().getColumn(RechnungFac.FLR_SPALTE_RE_UMSATZ_LEER1);
    tcEmpty.setCellRenderer(new LeeresFeldRenderer());
    tcEmpty.setPreferredWidth(SPALTENBREITE_LEERESPALTE);
    tcEmpty = getTable().getColumnModel().getColumn(RechnungFac.FLR_SPALTE_RE_UMSATZ_LEER2);
    tcEmpty.setCellRenderer(new LeeresFeldRenderer());
    tcEmpty.setPreferredWidth(SPALTENBREITE_LEERESPALTE);
  }
  ...
  protected void eventActionRefresh(ActionEvent e, boolean bNeedNoRefreshI) throws
    ...
    wlaKritAuswertung.setText(
        getDefaultFilter()[RechnungFac.IDX_KRIT_AUSWERTUNG]
