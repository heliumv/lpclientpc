...
public class ReportRechnungAlleRechnungen
extends PanelReportJournalVerkauf implements PanelReportIfJRDS
{
	...
	private WrapperCheckBox wcbGutschriftenBerucksichtigen = null;
	...
	public ReportRechnungAlleRechnungen(InternalFrame internalFrame, String add2Title)
	throws Throwable {
	...
	protected void jbInit() {
		...
		wrbVertreterAlle.setText(LPMain.getInstance().getTextRespectUISPr("label.alle"));
		wrbVertreterAlle.setActionCommand(ACTION_SPECIAL_VERTRETER_ALLE);
		wrbVertreterAlle.addActionListener(this);

		wrbVertreterEiner.setText(LPMain.getInstance().getTextRespectUISPr("label.einer"));
		...
		wcbStatistikadresse.setMinimumSize(new Dimension(200, Defaults.getInstance().getControlHeight()));
		wcbGutschriftenBerucksichtigen = new WrapperCheckBox();
		wcbGutschriftenBerucksichtigen.setText(LPMain.getTextRespectUISPr("rechnung.gutschriftenberuecksichtigen"));
		...
		jpaWorkingOn.add(wcbStatistikadresse,
				new GridBagConstraints(1, iZeile, 3, 1, 0.0, 0.0,
						...
						0, 0));
		iZeile++;
		jpaWorkingOn.add(wcbGutschriftenBerucksichtigen,
	...
	private void setDefaults() {
		...
		wcbGutschriftenBerucksichtigen.setSelected(true);
	}
	...
	public String getReportname() {
		return RechnungReportFac.REPORT_RECHNUNGEN_ALLE;
	}
	...
	protected ReportRechnungJournalKriterienDto getKriterien() {
		...
		krit.setBGutschriftenBeruecksichtigen(wcbGutschriftenBerucksichtigen.isSelected());
		return krit;
	}
	...
	throws Throwable {
		return DelegateFactory.getInstance().getRechnungDelegate().printRechnungenAlle(
				getKriterien());
	}
}
