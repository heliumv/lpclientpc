...
public class ReportRechnungOffene
// reportjournal: 1 das Reportpanel erbt PanelReportJournalEinkauf/Verkauf
// und implementiert PanelReportIfJRDS
	...
	private WrapperCheckBox wcbGutschriften = null;
	...
	public ReportRechnungOffene(InternalFrame internalFrame, String add2Title)
			throws Throwable {
		// reportjournal: 2 Der Titel, der im Internalframe angezeigt wird
	...
	protected void jbInit() throws Throwable {
		...
		wrbSortierungPartner.addItemListener(new ItemListener() {
			@Override
			public void itemStateChanged(ItemEvent e) {
				wcbNichtZugeordneteBelege.setVisible(wrbSortierungPartner.isSelected());
			}
		});

		// reportjournal: 3 Es koennen auch eigene zusaetzliche Felder
		// hinzugefuegt werden
		...
		wcbGutschriften = new WrapperCheckBox(
				LPMain.getTextRespectUISPr("rechnung.gutschriftenberuecksichtigen"));
		// default selektiert
		wcbGutschriften.setSelected(true);
		...
		wrbFaelligkeit = new WrapperRadioButton(LPMain.getInstance()
				.getTextRespectUISPr("lp.faelligkeit"));
		buttonGroupSortierung.add(wrbFaelligkeit);
		...
		iZeile++;
		jpaWorkingOn.add(wrbFaelligkeit, new GridBagConstraints(0, iZeile, 1,
				1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.BOTH,
				new Insets(2, 2, 2, 2), 0, 0));
		iZeile++;
		...
		jpaWorkingOn.add(wdfStichtag, new GridBagConstraints(1, iZeile, 1, 1,
				0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.BOTH,
				new Insets(2, 2, 2, 2), 0, 0));
		jpaWorkingOn.add(wcbGutschriften, new GridBagConstraints(2, iZeile, 1,
				1, 1.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.BOTH,
				new Insets(2, 2, 2, 2), 0, 0));
		jpaWorkingOn.add(wcbNichtZugeordneteBelege, new GridBagConstraints(3, iZeile, 1,
				1, 1.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.BOTH,
				new Insets(2, 2, 2, 2), 0, 0));
		
		iZeile++;
		jpaWorkingOn.add(wcbStatistikadresse, new GridBagConstraints(2, iZeile,
				1, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
	...
	protected ReportRechnungJournalKriterienDto getKriterien() {
		...
		if (wrbSortierungVertreter.isSelected()) {
			...
			krit.iSortierung = ReportJournalKriterienDto.KRIT_SORT_NACH_FAELLIGKEIT;
		}
		...
		krit.setbMitNichtZugeordnetendBelegen(wcbNichtZugeordneteBelege.isSelected());
		krit.setTStichtag(wdfStichtag.getDate());
		krit.setBGutschriftenBeruecksichtigen(wcbGutschriften.isSelected());
	...
	public String getReportname() {
		return RechnungReportFac.REPORT_RECHNUNGEN_OFFENE;
	}

	public JasperPrintLP getReport(String sDrucktype) throws Throwable {
		return DelegateFactory.getInstance().getRechnungDelegate()
				.printRechnungenOffene(getKriterien());
	}
	...
	public void aktiviereBeleg() throws Throwable {
	}
}
