 ...
 */
public class TabbedPaneLieferschein extends TabbedPaneBasis implements
		ICopyPaste {
	/**
	 * 
	 */
	...
	public final static String MY_OWN_NEW_AUS_AUFTRAG = PanelBasis.ACTION_MY_OWN_NEW
			+ EXTRA_NEU_AUS_AUFTRAG;
	public final static String MY_OWN_NEW_AUS_ANGEBOT = PanelBasis.ACTION_MY_OWN_NEW
	...
	public TabbedPaneLieferschein(InternalFrame internalFrameI)
			throws Throwable {
		super(internalFrameI, LPMain.getInstance().getTextRespectUISPr(
				"ls.modulname"));
		// Sammellieferschein aktiviert?
		bSammellieferschein = LPMain
				.getInstance()
				.getDesktop()
	...
	private void jbInit() throws Throwable {
		...
		insertTab(
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.panel.auswahl"),
				null,
				lsAuswahl,
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.tooltip.auswahl"),
				IDX_PANEL_LIEFERSCHEINAUSWAHL);

		// die restlichen Panels erst bei Bedarf laden
		insertTab(
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.panel.kopfdaten"),
				null,
				null,
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.tooltip.kopfdaten"),
				IDX_PANEL_LIEFERSCHEINKOPFDATEN);

		insertTab(
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.panel.positionen"),
				null,
				null,
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.tooltip.positionen"),
				IDX_PANEL_LIEFERSCHEINPOSITIONEN);
		// Der naechste Index
		...
		if (bSammellieferschein) {
			...
			insertTab(
					LPMain.getInstance().getTextRespectUISPr(
							"ls.title.panel.auftraege"),
					null,
					null,
					LPMain.getInstance().getTextRespectUISPr(
							"ls.title.tooltip.auftraege"),
					IDX_PANEL_LIEFERSCHEINAUFTRAEGE);
			index++;
		}
		...
		insertTab(
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.panel.sichtauftrag"),
				null,
				null,
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.tooltip.sichtauftrag"),
		...
		insertTab(
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.panel.konditionen"),
				null,
				null,
				LPMain.getInstance().getTextRespectUISPr(
						"ls.title.tooltip.konditionen"),
				IDX_PANEL_LIEFERSCHEINKONDITIONEN);
		index++;
		...
		insertTab(
				...
				null,
				LPMain.getInstance().getTextRespectUISPr("lp.umsatzuebersicht"),
				IDX_PANEL_LIEFERSCHEINUMSATZ);
		index++;

		createLieferscheinAuswahl();
		// den aktuell gewaehlten Lieferschein im Titel anzeigen
		setTitleLieferschein(LPMain.getInstance().getTextRespectUISPr(
				"ls.title.panel.auswahl"));
	...
	private void createLieferscheinAuswahl() throws Throwable {
		QueryType[] qtAuswahl = LieferscheinFilterFactory.getInstance()
				.createQTPanelLieferscheinauswahl();
		FilterKriterium[] filtersAuswahl = SystemFilterFactory.getInstance()
				.createFKMandantCNr();

		String[] aWhichButtonIUse = { PanelBasis.ACTION_NEW,
				PanelBasis.ACTION_FILTER };

		lsAuswahl = new PanelQuery(qtAuswahl, filtersAuswahl,
				QueryParameters.UC_ID_LIEFERSCHEIN, aWhichButtonIUse,
				getInternalFrame(),
				LPMain.getTextRespectUISPr("ls.title.panel.auswahl"), true); // liste
		// refresh
		// wenn
		// lasche
		// geklickt
		...
		FilterKriteriumDirekt fkDirekt1 = LieferscheinFilterFactory
				.getInstance().createFKDLieferscheinnummer();

		FilterKriteriumDirekt fkDirekt2 = LieferscheinFilterFactory
				.getInstance().createFKDKundenname();

		lsAuswahl.befuellePanelFilterkriterienDirekt(fkDirekt1, fkDirekt2);

		lsAuswahl.addDirektFilter(LieferscheinFilterFactory.getInstance()
				.createFKDProjekt());

		lsAuswahl.addDirektFilter(LieferscheinFilterFactory.getInstance()
				.createFKDAuftragsnummmer());

		lsAuswahl
				.befuelleFilterkriteriumSchnellansicht(LieferscheinFilterFactory
						.getInstance().createFKLieferscheineSchnellansicht());
		...
		if (selectedIndex == IDX_PANEL_LIEFERSCHEINAUSWAHL) {
			...
			if (getLieferscheinAuswahl().getSelectedId() == null) {
				getInternalFrame().enableAllOberePanelsExceptMe(this,
						IDX_PANEL_LIEFERSCHEINAUSWAHL, false);
			}
			...
			if (getAuftragDtoSichtAuftrag().getIId() == null
				...
				lsPositionenSichtAuftragTop
						.setDefaultFilter(LieferscheinFilterFactory
								.getInstance()
								.createFKLieferscheinSichtAuftrag(
										getLieferscheinDto(),
	...
	private void getKriterienLieferscheinUmsatz() throws Throwable {
		if (pdKriterienLsUmsatz == null) {
			pdKriterienLsUmsatz = new PanelDialogKriterienLieferscheinUmsatz(
					getInternalFrame(),
					LPMain.getInstance().getTextRespectUISPr(
	...
	private LieferscheinpositionDto getSelectedLieferscheinpositionDto()
			throws ExceptionLP {
		...
		return DelegateFactory
				...
				.lieferscheinpositionFindByPrimaryKey(
						(Integer) lsPositionenTop.getSelectedId());
	}
	...
	public void lPEventItemChanged(ItemChangedEvent e) throws Throwable {
		...
		if (e.getID() == ItemChangedEvent.GOTO_DETAIL_PANEL) {
			...
			if (e.getSource() == panelQueryFLRBegruendung) {
				...
				DelegateFactory
						.getInstance()
						.getLsDelegate()
						.updateLieferscheinBegruendung(
								getLieferscheinDto().getIId(), null);
			...
			if (e.getSource() == lsAuswahl) {
				...
				if (getLieferscheinDto().getIId() != null
					...
					lsPositionenSichtAuftragBottom
							.setKeyWhenDetailPanel(pkLieferschein);
				}
				...
				if (iIdAuftrag != null) {
					...
					if (null == lsPositionenSichtAuftragTop
							|| null == lsPositionenSichtAuftrag) {
						refreshLieferscheinPositionenSichtAuftrag();
					}
					lsPositionenSichtAuftragTop
							.setDefaultFilter(LieferscheinFilterFactory
			...
			if (e.getSource() == lsPositionenBottom) {
				...
				lsPositionenTop.updateButtons(new LockStateValue(
				...
				if (lsPositionenSichtAuftragBottom.getAuftragpositionDto() == null) {
					...
					getInternalFrame().enableAllOberePanelsExceptMe(this,
							IDX_PANEL_LIEFERSCHEINPOSITIONENSICHTAUFTRAG, true);
				} else {
					// im QP die Buttons in den Zustand neu setzen.
					lsPositionenSichtAuftragTop
					...
					if (istAktualisierenLieferscheinErlaubt()) {
						if (sAspectInfo.equals(MY_OWN_NEW_SNRBARCODEERFASSUNG)) {
							...
							if (alSeriennummern.size() > 0
								...
								lsPos.setBNettopreisuebersteuert(Helper
										.boolean2Short(false));
								...
								lsPos.setMwstsatzIId(mwstsatzDtoAktuell
										.getIId());
								...
								if (verkaufspreisDtoInZielwaehrung != null) {
									...
									lsPos.setNBruttoeinzelpreis(new BigDecimal(
											0));
								}

								lsPos.setFRabattsatz(0D);
								lsPos.setFZusatzrabattsatz(0D);
								lsPos.setNMenge(new BigDecimal(alSeriennummern
										.size()));
								...
								DelegateFactory
										.getInstance()
										.getLieferscheinpositionDelegate()
										.createLieferscheinposition(lsPos,
												false);
			...
			if (e.getSource() == lsAuswahl) {
				...
				if (lsAuswahl.getSelectedId() == null) {
					getInternalFrame().enableAllOberePanelsExceptMe(this,
							IDX_PANEL_LIEFERSCHEINAUSWAHL, true);
				}
		...
		if (e.getID() == ItemChangedEvent.ACTION_GOTO_MY_DEFAULT_QP) {
			if (e.getSource() == lsPositionenBottom) {
				...
				if (lsPositionenBottom.getKeyWhenDetailPanel() == null) {
					Object oNaechster = lsPositionenTop
		...
		if (e.getID() == ItemChangedEvent.ACTION_SAVE) {
			...
			if (e.getSource() == lsKopfdaten) {
				...
				getInternalFrame()
						.setKeyWasForLockMe(pkLieferschein.toString());
		...
		if (e.getID() == ItemChangedEvent.ACTION_KRITERIEN_HAVE_BEEN_SELECTED) {
			...
			if (e.getSource() == pdKriterienLsUebersicht) { // ptclient: 5
				...
				FilterKriterium[] aAlleKriterien = pdKriterienLsUebersicht
				...
				setTitleLieferscheinOhneLieferscheinnummer(LPMain.getInstance()
						.getTextRespectUISPr("lp.umsatzuebersicht"));
				ptLieferscheinUebersicht.updateButtons(new LockStateValue(null,
						null, PanelBasis.LOCK_IS_NOT_LOCKED));
			} else

			if (e.getSource() == pdKriterienLsUmsatz) {
				// die Kriterien fuer PanelTabelle abholen
				FilterKriterium[] aAlleKriterien = pdKriterienLsUmsatz
						.getAlleFilterKriterien();
				...
				ptLieferscheinUmsatz.updateButtons(new LockStateValue(null,
						null, PanelBasis.LOCK_IS_NOT_LOCKED));
			}
		} else
		...
		if (e.getID() == ItemChangedEvent.ACTION_POSITION_VONNNACHNMINUS1) {
			if (e.getSource() == lsPositionenTop) {
				if (getLieferscheinDto().getStatusCNr().equals(
					...
					if (iPos > 0) {
						Integer iIdPosition = (Integer) lsPositionenTop
								.getSelectedId();
		...
		if (e.getID() == ItemChangedEvent.ACTION_POSITION_VORPOSITIONEINFUEGEN) {
			if (e.getSource() == lsPositionenTop) {
				...
				if (lsPositionenTop.getSelectedId() != null) {
					LieferscheinpositionDto lsPos = DelegateFactory
							.getInstance()
							.getLieferscheinpositionDelegate()
							.lieferscheinpositionFindByPrimaryKey(
									(Integer) lsPositionenTop.getSelectedId());

					lsPositionenBottom.setArtikeSetIIdForNewPosition(lsPos
		...
		if (e.getSource() == lsAuswahl) {
			...
			if (iIdAuftragBasis != null) {
				...
				boolean b = DelegateFactory
						.getInstance()
						.getKundeDelegate()
						.pruefeKreditlimitMitJaNeinFrage(getInternalFrame(),
				...
				if (b == true) {
					Integer lieferscheinIId = DelegateFactory
							.getInstance()
							.getAuftragDelegate()
							.erzeugeLieferscheinAusAuftrag(iIdAuftragBasis,
									null);
					...
					if (bSammellieferschein) {
						...
						setAuftragDtoSichtAuftrag(DelegateFactory.getInstance()
								.getAuftragDelegate()
								.auftragFindByPrimaryKey(iIdAuftragBasis));
			...
			if (iIdAuftrag != null) {
				...
				// PJ17887
				if (e.getSource() == panelQueryFLRAuftragauswahlZusatz) {
					DialogUnterschiedlicheKonditionen d = new DialogUnterschiedlicheKonditionen(
							auftragDto, getLieferscheinDto(),
							(InternalFrameLieferschein) getInternalFrame());
					LPMain.getInstance().getDesktop()
							.platziereDialogInDerMitteDesFensters(d);
					if (d.bKonditionenUnterschiedlich == true) {
						...
						setLieferscheinDto(DelegateFactory
								.getInstance()
								.getLsDelegate()
			...
			if (iIdAngebotBasis != null) {
				...
				AngebotDto aDto = DelegateFactory.getInstance()
						.getAngebotDelegate()
				...
				boolean b = DelegateFactory
						...
						.getKundeDelegate()
						.pruefeKreditlimitMitJaNeinFrage(getInternalFrame(),
								aDto.getKundeIIdAngebotsadresse());

				if (b == true) {

					Integer lieferscheinIId = DelegateFactory.getInstance()
							.getAngebotDelegate()
							.erzeugeLieferscheinAusAngebot(iIdAngebotBasis);
		...
		panelQueryFLRAngebotauswahl = AngebotFilterFactory.getInstance()
				.createPanelFLRAngebot(
						...
						AngebotFilterFactory.getInstance()
								.createFKAngebotNichtAngelegt()); // pro Angebot
		// 1 Auftrag
		// & 1
		// Lieferschein
		// -> FK
	...
	private PanelBasis refreshLieferscheinKopfdaten() throws Throwable {
		...
		if (lsKopfdaten == null) {
			...
			if (getInternalFrame().getKeyWasForLockMe() != null) {
				iIdLieferschein = new Integer(
						Integer.parseInt(getInternalFrame()
								.getKeyWasForLockMe()));
			}

			lsKopfdaten = new PanelLieferscheinKopfdaten(getInternalFrame(),
					LPMain.getInstance().getTextRespectUISPr(
							"ls.title.panel.kopfdaten"), iIdLieferschein); // empty
			// bei
			// leerer
			// Liste
	...
	public PanelSplit refreshLieferscheinPositionen() throws Throwable {
		if (lsPositionen == null) {
			lsPositionenBottom = new PanelLieferscheinPositionen(
					getInternalFrame(), LPMain.getInstance()
							.getTextRespectUISPr("ls.title.panel.positionen"),
					null); // default belegung, eventuell gibt es noch keine
			// position
			...
			lsPositionenTop = new PanelQuery(qtPos, filtersPos,
			...
			lsPositionen = new PanelSplit(getInternalFrame(),
					lsPositionenBottom, lsPositionenTop, 165);
	...
	public PanelQuery getLieferscheinAuftraege() throws Throwable {
		if (lsAuftraege == null) {
			QueryType[] qtPos = null;
			FilterKriterium[] filtersPos = LieferscheinFilterFactory
					.getInstance().createFKAuftraegeEinesLieferscheins(
							getLieferscheinDto().getIId());

			String[] aWhichButtonIUse = { PanelBasis.ACTION_NEW };

			lsAuftraege = new PanelQuery(qtPos, filtersPos,
					QueryParameters.UC_ID_AUFTRAEGE_EINES_LIEFERSCHEINS,
					aWhichButtonIUse, getInternalFrame(), LPMain.getInstance()
							.getTextRespectUISPr("ls.title.panel.auftraege"),
					true);
			setComponentAt(IDX_PANEL_LIEFERSCHEINAUFTRAEGE, lsAuftraege);
		}
		this.setTitleLieferschein(LPMain.getInstance().getTextRespectUISPr(
				"ls.title.panel.auftraege"));
	...
	private PanelBasis getLieferscheinKonditionen() throws Throwable {
		if (lsKonditionen == null) {
			lsKonditionen = new PanelLieferscheinKonditionen2(
					getInternalFrame(), LPMain.getInstance()
	 ...
	 * @throws Throwable
	 */
	public PanelSplit refreshLieferscheinPositionenSichtAuftrag()
			throws Throwable {
		if (lsPositionenSichtAuftrag == null) {
			lsPositionenSichtAuftragBottom = new PanelLieferscheinPositionenSichtAuftrag(
					getInternalFrame(),
					LPMain.getTextRespectUISPr("ls.title.panel.sichtauftrag"),
					null, getLieferscheinDto().getLagerIId());

			FilterKriterium[] filtersPos = LieferscheinFilterFactory
					.getInstance().createFKLieferscheinSichtAuftrag(
							getLieferscheinDto(),
							getAuftragDtoSichtAuftrag().getIId());
			...
			setComponentAt(IDX_PANEL_LIEFERSCHEINPOSITIONENSICHTAUFTRAG,
					lsPositionenSichtAuftrag);
		}
	...
	public void initializeDtos(Integer iIdLieferscheinI) throws Throwable {
		if (iIdLieferscheinI != null) {
			...
			kundeLieferadresseDto = DelegateFactory
					.getInstance()
					.getKundeDelegate()
					.kundeFindByPrimaryKey(
			...
			if (getLieferscheinDto().getLieferscheintextIIdDefaultKopftext() == null
					&& getLieferscheinDto()
							.getLieferscheintextIIdDefaultFusstext() == null) {
			...
			if (getLieferscheinDto().getLieferscheintextIIdDefaultKopftext() != null) {
				kopftextDto = DelegateFactory
						...
						.lieferscheintextFindByPrimaryKey(
								getLieferscheinDto()
			...
			if (!bSammellieferschein) {
				if (getLieferscheinDto().getAuftragIId() != null) {
					setAuftragDtoSichtAuftrag(DelegateFactory
							.getInstance()
							.getAuftragDelegate()
							.auftragFindByPrimaryKey(
									getLieferscheinDto().getAuftragIId()));
				} else {
					setAuftragDtoSichtAuftrag(new AuftragDto());
				}
			}
	...
	public void setTitleLieferschein(String panelTitle) throws Throwable {
		...
		if (getLieferscheinDto() == null
				|| getLieferscheinDto().getIId() == null) {
			lsnummer.append(LPMain.getInstance().getTextRespectUISPr(
					"ls.title.neuerlieferschein"));
		} else {
			KundeDto kundeDto = DelegateFactory
					.getInstance()
					.getKundeDelegate()
			...
			lsnummer.append(
					LPMain.getInstance().getTextRespectUISPr(
							"ls.title.lieferschein"))
					.append(" ")
					.append(getLieferscheinDto().getCNr())
					.append(" ")
	...
	public void setTitleLieferscheinOhneLieferscheinnummer(String panelTitle)
			throws Exception {
		// dem Panel die Titel setzen
		getInternalFrame().setLpTitle(InternalFrame.TITLE_IDX_OHRWASCHLOBEN,
				panelTitle);
	 ...
	 *             Ausnahme
	 */
	public LieferscheintextDto getDefaultFusstext(String sLocAsString)
			throws Throwable {
		return DelegateFactory
				.getInstance()
				.getLieferscheinServiceDelegate()
				.lieferscheintextFindByMandantLocaleCNr(sLocAsString,
	 ...
	 *             Ausnahme
	 */
	public LieferscheintextDto getDefaultKopftext(String sLocAsString)
			throws Throwable {
		return DelegateFactory
				.getInstance()
				.getLieferscheinServiceDelegate()
	...
	public boolean pruefeAktuellenLieferschein() {
		...
		if (getLieferscheinDto() == null
			...
			DialogFactory.showModalDialog(LPMain.getInstance()
					...
					.getTextRespectUISPr("ls.warning.keinlieferschein"));
		}
	...
	private boolean pruefeKonditionen() throws Throwable {
		...
		if (pruefeAktuellenLieferschein()) {
			if (Helper.short2boolean(sValue)) {
				if (getLieferscheinDto()
						.getLieferscheintextIIdDefaultKopftext() == null
						&& getLieferscheinDto()
								.getLieferscheintextIIdDefaultFusstext() == null) {
					bErfasst = false;
	...
	public void initLieferscheinKonditionen() throws Throwable {
		...
		getLieferscheinDto().setLieferscheintextIIdDefaultKopftext(
				kopftextDto.getIId());
		getLieferscheinDto().setLieferscheintextIIdDefaultFusstext(
				fusstextDto.getIId());

		DelegateFactory.getInstance().getLsDelegate()
				.updateLieferscheinOhneWeitereAktion(getLieferscheinDto());
	}
		...
		if (kopftextDto == null || kopftextDto.getIId() == null) {
			try {
				...
				kopftextDto = DelegateFactory
						.getInstance()
						.getLieferscheinServiceDelegate()
						.createDefaultLieferscheintext(
								MediaFac.MEDIAART_KOPFTEXT, localeCNr);
			}
		}
		...
		if (fusstextDto == null || fusstextDto.getIId() == null) {
			try {
				...
				fusstextDto = DelegateFactory
						...
						.getLieferscheinServiceDelegate()
						.createDefaultLieferscheintext(
								MediaFac.MEDIAART_FUSSTEXT, localeCNr);
			}
		}
	}
	...
	public boolean esGibtOffeneLieferscheine() throws Throwable {
		...
		if (DelegateFactory.getInstance().getLsDelegate()
				.berechneAnzahlDerOffenenLieferscheine() <= 0) {
			bGibtOffene = false;
	...
	public boolean aktuellerLieferscheinHatPositionen() throws Throwable {
		...
		if (DelegateFactory
				...
				.berechneAnzahlMengenbehaftetePositionen(
						getLieferscheinDto().getIId()) <= 0) {
			bGibtPositionen = false;
			DialogFactory.showModalDialog(LPMain.getInstance()
					.getTextRespectUISPr("lp.warning"), LPMain.getInstance()
					.getTextRespectUISPr("ls.warning.keinepositionen"));
		}
	...
	protected void lPActionEvent(ActionEvent e) throws Throwable {
		...
		if (e.getActionCommand().equals(MENU_ACTION_DATEI_LIEFERSCHEIN)) {
			printLieferschein();
				...
				MENU_ACTION_DATEI_LIEFERSCHEINETIKETT)) {
			printLieferscheinetikett();
		} else if (e.getActionCommand().equals(
				MENU_ACTION_DATEI_LIEFERSCHEINWAETIKETT)) {
			printLieferscheinwaetikett(null);
		} else if (e.getActionCommand().equals(
				MENU_ACTION_DATEI_VERANDETIKETTEN)) {
			ReportVersandetiketten ve = new ReportVersandetiketten(
					getInternalFrame(), getLieferscheinDto(), getTitleDruck());
			...
			getInternalFrame().showReportKriterien(
					new ReportLieferscheinAlleLieferscheine(getInternalFrame(),
							LPMain.getInstance().getTextRespectUISPr(
									"ls.menu.journal.alle")),
					getKundeLieferadresseDto().getPartnerDto(),
					getLieferscheinDto().getAnsprechpartnerIId(), false);
		} else if (e.getActionCommand().equals(MENU_ACTION_JOURNAL_OFFEN)) {
			getInternalFrame().showReportKriterien(
					new ReportLieferscheinOffene(getInternalFrame(), LPMain
							.getInstance().getTextRespectUISPr(
									"ls.print.listeoffene")),
					getKundeLieferadresseDto().getPartnerDto(),
					getLieferscheinDto().getAnsprechpartnerIId(), false);
		} else
		...
		if (e.getActionCommand().equals(MENU_ACTION_JOURNAL_UEBERSICHT)) {
			...
			getInternalFrame().showPanelDialog(pdKriterienLsUebersicht);
				...
				MENU_ACTION_JOURNAL_UMSATZUEBERSICHT)) {
			getKriterienLieferscheinUmsatz();
			getInternalFrame().showPanelDialog(pdKriterienLsUmsatz);
				...
				if (!refreshLieferscheinKopfdaten().isLockedDlg()) {
					...
					if (getLieferscheinDto().getStatusCNr().equals(
						...
						showStatusMessage("lp.warning",
								"ls.warning.kannnichterledigtwerden");
					}
				}
			}
		}
	...
	protected boolean handleOwnException(ExceptionLP exfc) {
		...
		case EJBExceptionLP.ARTIKEL_KEINE_LAGERBUCHUNG_VORHANDEN:
			DialogFactory.showModalDialog(LPMain.getInstance()
					.getTextRespectUISPr("lp.error"), LPMain.getInstance()
					.getTextRespectUISPr("ls.error.lagerbuchung"));
			break;
	...
	public void printLieferschein() throws Throwable {
		...
		if (pruefeKonditionen()) {
			...
			if (!refreshLieferscheinKopfdaten().isLockedDlg()) {
				getInternalFrame().showReportKriterien(
						new ReportLieferschein(getInternalFrame(),
								getAktuellesPanel(), getLieferscheinDto(),
								getTitleDruck()),
						getKundeLieferadresseDto().getPartnerDto(),
						getLieferscheinDto().getAnsprechpartnerIId(), false);
			}
	...
	public void printLieferscheinetikett() throws Throwable {
		...
		if (pruefeAktuellenLieferschein()) {
			if (aktuellerLieferscheinHatPositionen()) {
				boolean bHatVersandRecht = DelegateFactory.getInstance()
						.getTheJudgeDelegate()
						.hatRecht(RechteFac.RECHT_LS_LIEFERSCHEIN_VERSAND);
				ReportLieferscheinAdressetikett rla = new ReportLieferscheinAdressetikett(
						getInternalFrame(), getLieferscheinDto(),
						getTitleDruck());
				rla.eventYouAreSelected(false);
				rla.setKeyWhenDetailPanel(getLieferscheinDto().getIId());
				getInternalFrame().showReportKriterien(rla,
						getKundeLieferadresseDto().getPartnerDto(),
						getLieferscheinDto().getAnsprechpartnerIId(), false);
	...
	public void printLieferscheinwaetikett(Integer lieferscheinpositionIId)
			throws Throwable {

		if (pruefeAktuellenLieferschein()) {
			if (aktuellerLieferscheinHatPositionen()) {
				ReportLieferscheinWAEetikett rwae = new ReportLieferscheinWAEetikett(
						getInternalFrame(), getLieferscheinDto(),
						lieferscheinpositionIId, getTitleDruck());
				rwae.eventYouAreSelected(false);
				rwae.setKeyWhenDetailPanel(getLieferscheinDto().getIId());
				// man kann unabhaengig vom Status beliebig oft drucken
				getInternalFrame().showReportKriterien(rwae,
						getKundeLieferadresseDto().getPartnerDto(),
						getLieferscheinDto().getAnsprechpartnerIId(), false);
				rwae.updateButtons();
			}
	...
	private String getTitleDruck() {
		...
		buff.append(" ");
		buff.append(getKundeLieferadresseDto().getPartnerDto()
	...
	public void dialogQueryAuftragFromListeZusatz(ActionEvent e)
			throws Throwable {
		// Der muss die gleichen Adressen haben wie der Hauptauftrag
		if (getLieferscheinDto().getAuftragIId() != null) {
			AuftragDto auftragDtoHA = DelegateFactory
					.getInstance()
					.getAuftragDelegate()
					.auftragFindByPrimaryKey(
							getLieferscheinDto().getAuftragIId());
			...
			Integer[] auftragIIdNA = new Integer[getLieferscheinAuftraege()
			...
			for (int i = 0; i < getLieferscheinAuftraege().getTable()
					.getRowCount(); i++) {
				auftragIIdNA[i + 1] = (Integer) getLieferscheinAuftraege()
	...
	public LagerDto getHauptlagerDesMandanten() throws Throwable {
		LagerDto oLagerDto = DelegateFactory.getInstance().getLagerDelegate()
	...
	public boolean istLieferscheinAuftragbezogen() throws Throwable {
		...
		if (pruefeAktuellenLieferschein()) {
			if (getLieferscheinDto().getLieferscheinartCNr().equals(
					LieferscheinFac.LSART_AUFTRAG)) {
				DialogFactory.showModalDialog(
						LPMain.getInstance().getTextRespectUISPr("lp.warning"),
						LPMain.getInstance().getTextRespectUISPr(
								"ls.warning.lsistauftragbezogen"));
	...
	private PanelTabelle getLieferscheinUmsatz() throws Throwable {
		if (ptLieferscheinUmsatz == null) {
			ptLieferscheinUmsatz = new PanelTabelleLieferscheinUmsatz(
					QueryParameters.UC_ID_LIEFERSCHEINUMSATZ, LPMain
							.getInstance().getTextRespectUISPr(
	...
	protected javax.swing.JMenuBar getJMenuBar() throws Throwable {
		...
		JMenuItem menuItemDateiVersandetikett = new JMenuItem(LPMain
				.getInstance().getTextRespectUISPr("ls.versandetiketten")
				+ "...");
		menuItemDateiVersandetikett.addActionListener(this);
		menuItemDateiVersandetikett
				.setActionCommand(MENU_ACTION_DATEI_VERANDETIKETTEN);
		jmModul.add(menuItemDateiVersandetikett, 0);
		JMenuItem menuItemDateiLieferscheinWAEtikett = new JMenuItem(LPMain
				.getInstance().getTextRespectUISPr(
						"ls.menu.datei.lieferscheinwaetikett"));
		menuItemDateiLieferscheinWAEtikett.addActionListener(this);
		menuItemDateiLieferscheinWAEtikett
				.setActionCommand(MENU_ACTION_DATEI_LIEFERSCHEINWAETIKETT);
		jmModul.add(menuItemDateiLieferscheinWAEtikett, 0);

		JMenuItem menuItemDateiLieferscheinetikett = new JMenuItem(LPMain
				.getInstance().getTextRespectUISPr(
						"ls.menu.datei.lieferscheinetikett"));
		menuItemDateiLieferscheinetikett.addActionListener(this);
		menuItemDateiLieferscheinetikett
				.setActionCommand(MENU_ACTION_DATEI_LIEFERSCHEINETIKETT);
		...
		menuItemDateiLieferschein
				.setActionCommand(MENU_ACTION_DATEI_LIEFERSCHEIN);
		...
		JMenuItem menuItemBearbeitenBegruendung = new JMenuItem(LPMain
				.getInstance().getTextRespectUISPr("ls.begruendung"));
		...
		JMenuItem menuItemJournalAngelegt = new JMenuItem(LPMain.getInstance()
				.getTextRespectUISPr("ls.menu.journal.angelegt"));
		...
		JMenuItem menuItemJournalAlle = new JMenuItem(LPMain.getInstance()
				.getTextRespectUISPr("ls.menu.journal.alle"));
		...
		JMenuItem menuItemJournalOffene = new JMenuItem(LPMain.getInstance()
				.getTextRespectUISPr("ls.menu.journal.offen"));
		...
		WrapperMenuItem menuItemJournalUmsatzTag = new WrapperMenuItem(LPMain
		...
		menuItemJournalUmsatzTag
				.setActionCommand(MENU_ACTION_JOURNAL_UMSATZUEBERSICHT);
	...
	public String getLieferscheinStatus() throws Throwable {
		StatusDto statusDto = DelegateFactory
				.getInstance()
				.getLocaleDelegate()
				.statusFindByPrimaryKey(
						getLieferscheinDto().getStatusCNr());
	...
	public void enablePanels(LieferscheinDto lsDtoI, boolean bDisableOnlyI)
			throws Throwable {
		if (lsDtoI.getIId() != null) {
			...
			if (cLsArt.equals(LieferscheinFac.LSART_LIEFERANT)
					|| cLsArt.equals(LieferscheinFac.LSART_FREI)) {
				enableI = false;
			}
			if (!bDarfPreiseSehen)
	...
	public void copyHV() throws ExceptionLP, Throwable {
		...
		if (aoIIdPosition != null && (aoIIdPosition.length > 0)) {
			...
			if (lsPositionenBottom.getArtikelsetViewController()
	...
	public void einfuegenHV() throws IOException, ExceptionLP,
		...
		if (o instanceof BelegpositionDto[]) {
			LieferscheinpositionDto[] lspositionDtos = DelegateFactory
					.getInstance()
				...
				if (b != null) {
					for (int i = 0; i < lspositionDtos.length; i++) {
						...
						try {
							...
							if (b == false) {
								Integer iIdAktuellePosition = (Integer) getLieferscheinPositionenTop()
										.getSelectedId();
								...
								if (iIdAktuellePosition != null) {
									LieferscheinpositionDto aktuellePositionDto = DelegateFactory
											.getInstance()
											.getLieferscheinpositionDelegate()
											.lieferscheinpositionFindByPrimaryKey(
													iIdAktuellePosition);
							...
							positionDto.setBelegIId(getLieferscheinDto()
									.getIId());

							if (positionDto.getLieferscheinpositionartCNr()
								...
								if (artikelDto != null
									...
									fehlerMeldungDto
											.setLieferscheinIId(positionDto
													.getLieferscheinIId());
									fehlerMeldungDto
								...
								// Chargennnummernbehaftet
									...
									// muss.
									SeriennrChargennrAufLagerDto[] dtos = DelegateFactory
											.getInstance()
											.getLagerDelegate()
											.getAllSerienChargennrAufLager(
													positionDto.getArtikelIId(),
													this.getLieferscheinDto()
															.getLagerIId());
									BigDecimal nLagerstd = dtos.length > 0 ? dtos[0]
											.getNMenge() : BigDecimal.ONE
											.negate();
									if (nLagerstd.compareTo(positionDto
											.getNMenge()) >= 0) {
							...
							if (iId == null) {
								...
								if (bDiePositionSpeichern) {
									...
									if (bDiePositionSpeichern) {
										...
										lspositionDtos[i] = DelegateFactory
												.getInstance()
												.getLieferscheinpositionDelegate()
												.lieferscheinpositionFindByPrimaryKey(
														iId);
	...
	public void fillMustFields(BelegpositionDto belegposDtoI, int xalOfBelegPosI)
		...
		if (lieferscheinpositionDtoI.getBArtikelbezeichnunguebersteuert() == null) {
			lieferscheinpositionDtoI.setBArtikelbezeichnunguebersteuert(Helper
					.boolean2Short(false));
		}
	}
}
