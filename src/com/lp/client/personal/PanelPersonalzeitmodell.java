...
public class PanelPersonalzeitmodell extends PanelBasis {
	...
	private PersonalzeitmodellDto personalzeitmodellDto = null;
	...
	public PanelPersonalzeitmodell(InternalFrame internalFrame,
	...
	void dialogQueryZeitmodellFromListe(ActionEvent e) throws Throwable {
		panelQueryFLRZeitmodell = PersonalFilterFactory.getInstance()
				.createPanelFLRZeitmodell(getInternalFrame(),
						personalzeitmodellDto.getZeitmodellIId(), false);
	...
	public void eventActionNew(EventObject eventObject, boolean bLockMeI,
		...
		personalzeitmodellDto = new PersonalzeitmodellDto();
	...
	protected void eventActionDelete(ActionEvent e,
		...
		DelegateFactory.getInstance().getPersonalDelegate()
				.removePersonalzeitmodell(personalzeitmodellDto);
		this.setKeyWhenDetailPanel(null);
		super.eventActionDelete(e, false, false);
	}

	protected void components2Dto() {
		personalzeitmodellDto.setTGueltigab(wdfGueltigab.getTimestamp());
		personalzeitmodellDto.setPersonalIId(internalFramePersonal
				.getPersonalDto().getIId());
	}

	protected void dto2Components() {
		wdfGueltigab.setTimestamp(personalzeitmodellDto.getTGueltigab());
		wtfZeitmodell
				.setText(personalzeitmodellDto.getZeitmodellDto().getCNr());
		if (personalzeitmodellDto.getZeitmodellDto().getZeitmodellsprDto() != null) {
			wtfZeitmodellBezeichnung.setText(personalzeitmodellDto
					.getZeitmodellDto().getZeitmodellsprDto().getCBez());
		} else {
		...
		if (allMandatoryFieldsSetDlg()) {
			...
			if (personalzeitmodellDto.getIId() == null) {
				...
				if (iAnzahl == 0) {
					...
					c.setTimeInMillis(personalzeitmodellDto.getTGueltigab()
							.getTime());
				...
				personalzeitmodellDto.setIId(DelegateFactory.getInstance()
						.getPersonalDelegate()
						.createPersonalzeitmodell(personalzeitmodellDto));
				setKeyWhenDetailPanel(personalzeitmodellDto.getIId());
			} else {
				DelegateFactory.getInstance().getPersonalDelegate()
						.updatePersonalzeitmodell(personalzeitmodellDto);
			}
	...
	protected void eventItemchanged(EventObject eI) throws Throwable {
		...
		if (e.getID() == ItemChangedEvent.GOTO_DETAIL_PANEL) {
			if (e.getSource() == panelQueryFLRZeitmodell) {
				...
				personalzeitmodellDto.setZeitmodellIId(zeitmodellDto.getIId());
			}
		}
	}
	...
	public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI)
		...
		if (key == null || (key.equals(LPMain.getLockMeForNew()))) {
			...
			personalzeitmodellDto = DelegateFactory.getInstance()
					.getPersonalDelegate()
					.personalzeitmodellFindByPrimaryKey((Integer) key);
