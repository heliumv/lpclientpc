 ...
 */
public class PanelDialogPersonalKommentar
...
/** Cache for convenience. */
  private InternalFramePersonal intFrame = null;
  ...
  public PanelDialogPersonalKommentar(InternalFrame internalFrame,
                                      String add2TitleI, boolean bInternerKommentarI)
      throws Throwable {
    super(internalFrame, add2TitleI);

    intFrame = (InternalFramePersonal) getInternalFrame();
  ...
  private void jbInit()
    ...
    if (intFrame.getPersonalDto() != null) {
      lockMePersonal = new LockMeDto(
          ...
          intFrame.getPersonalDto().getIId() + "",
  ...
  private void dto2Components()
    ...
    if (this.bInternerKommentar) {
      getLpEditor().setText(intFrame.getPersonalDto().getXKommentar());
      setCBestehenderKommentar(intFrame.getPersonalDto().getXKommentar());
    }
    else {
      getLpEditor().setText(intFrame.getPersonalDto().getXKommentar());
      setCBestehenderKommentar(intFrame.getPersonalDto().getXKommentar());
  ...
  public void saveKommentar()
    ...
    if (this.bInternerKommentar) {
      intFrame.getPersonalDto().setXKommentar(getLpEditor().getText());
    }
    else {
      intFrame.getPersonalDto().setXKommentar(getLpEditor().getText());
    }

    DelegateFactory.getInstance().getPersonalDelegate().updatePersonalKommentar(intFrame.getPersonalDto());
  }
