...
public class PanelZeitmodelltagpause
    extends PanelBasis
{
...
//von hier ...
  ...
  private ZeitmodelltagpauseDto zeitmodelltagpauseDto = null;
  public ZeitmodelltagDto zeitmodelltagDto = null;
  ...
  public PanelZeitmodelltagpause(InternalFrame internalFrame, String add2TitleI,
                                 Object pk)
      throws Throwable {
  ...
  protected void eventActionDelete(ActionEvent e, boolean bAdministrateLockKeyI,
      ...
      throws Throwable {
    DelegateFactory.getInstance().getZeiterfassungDelegate().removeZeitmodelltagpause(
  ...
  protected void dto2Components()
    ...
    wtfBeginn.setTime(zeitmodelltagpauseDto.getUBeginn());
    wtfEnde.setTime(zeitmodelltagpauseDto.getUEnde());

    this.setStatusbarPersonalIIdAendern(zeitmodelltagpauseDto.getPersonalIIdAendern());
    this.setStatusbarTAendern(zeitmodelltagpauseDto.getTAendern());
    ...
    if (allMandatoryFieldsSetDlg()) {
      if (wtfBeginn.getTime().before(wtfEnde.getTime())) {
        ...
        if (zeitmodelltagpauseDto.getIId() == null) {
          zeitmodelltagpauseDto.setIId(DelegateFactory.getInstance().
    ...
    if (zeitmodelltagDto != null) {
      String s = DelegateFactory.getInstance().getZeiterfassungDelegate().
          tagesartFindByPrimaryKey(
              zeitmodelltagDto.getTagesartIId()).getCNr();
      ...
      if (key == null
        ...
        zeitmodelltagpauseDto = DelegateFactory.getInstance().getZeiterfassungDelegate().
