...
public class PanelZeitabschluss extends PanelBasis {
	...
	private ZeitabschlussDto zeitabschlussDto = null;

	public PanelZeitabschluss(InternalFrame internalFrame, String add2TitleI,
	...
	public void eventActionNew(EventObject eventObject, boolean bLockMeI,
		...
		zeitabschlussDto = new ZeitabschlussDto();
	...
	protected void eventActionDelete(ActionEvent e,
		...
		if (pruefeObBuchungMoeglich()) {
			DelegateFactory.getInstance().getPersonalDelegate()
					.removeZeitabschluss(zeitabschlussDto);
	...
	protected void components2Dto() throws ExceptionLP {
		zeitabschlussDto.setTAbgeschlossenBis(wdfVon.getTimestamp());

		zeitabschlussDto.setPersonalIId(internalFrameZeiterfassung
				.getPersonalDto().getIId());

	}

	protected void dto2Components() throws ExceptionLP, Throwable {
		wdfVon.setTimestamp(zeitabschlussDto.getTAbgeschlossenBis());
	...
	public LockStateValue getLockedstateDetailMainKey() throws Throwable {
		...
		if (getKeyWhenDetailPanel() instanceof Integer) {
			...
			zeitabschlussDto = DelegateFactory.getInstance()
					.getPersonalDelegate().zeitabschlussFindByPrimaryKey(key);

			boolean bZeitenAbschliessenCUD = DelegateFactory.getInstance()
					.getTheJudgeDelegate()
					.hatRecht(RechteFac.RECHT_PERS_ZEITEN_ABSCHLIESSEN_CUD);

			if (bZeitenAbschliessenCUD == false) {
	...
	public void eventActionSave(ActionEvent e, boolean bNeedNoSaveI)
		...
		if (allMandatoryFieldsSetDlg()) {
			...
			if (pruefeObBuchungMoeglich()) {
				...
				if (zeitabschlussDto.getIId() == null) {

					zeitabschlussDto.setIId(DelegateFactory.getInstance()
							.getPersonalDelegate()
							.createZeitabschluss(zeitabschlussDto));
					setKeyWhenDetailPanel(zeitabschlussDto.getIId());
				} else {
					DelegateFactory.getInstance().getPersonalDelegate()
							.updateZeitabschluss(zeitabschlussDto);
				}
	...
	private void jbInit() throws Throwable {
		...
		wlaVon.setText(LPMain
				.getTextRespectUISPr("zeiterfassung.zeitabschluss.abgeschlossen_bis"));
	...
	public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI)
		...
		if (key == null || (key.equals(LPMain.getLockMeForNew()))) {
			...
			zeitabschlussDto = DelegateFactory.getInstance()
					.getPersonalDelegate()
					.zeitabschlussFindByPrimaryKey((Integer) key);
			dto2Components();
		}
	}
