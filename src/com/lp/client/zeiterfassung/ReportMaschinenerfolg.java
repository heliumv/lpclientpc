...
public class ReportMaschinenerfolg extends PanelBasis implements
		PanelReportIfJRDS {
	/**
	 * 
	 */
	...
	private WrapperCheckBox wcbMonatsbetrachtung = new WrapperCheckBox();
	...
	public ReportMaschinenerfolg(InternalFrameZeiterfassung internalFrame,
		...
		c.set(c.DAY_OF_MONTH, c.get(c.DAY_OF_MONTH));
		c.set(c.MINUTE, 0);
		c.set(c.HOUR, 0);
		c.set(c.SECOND, 0);
	...
	private void jbInit() throws Throwable {
		...
		wcbMonatsbetrachtung.setText(LPMain.getInstance().getTextRespectUISPr("report.maschinenerfolg.monatsbetrachtung"));
		...
		wcbNurSelektiertePerson
				.setText(LPMain
						.getTextRespectUISPr("zeiterfassung.report.monatsabrechnung.selektierteperson"));
		wcbNurSelektiertePerson.setSelected(true);
		...
		jpaWorkingOn.add(wrbSortPersonalnummer, new GridBagConstraints(3,
				...
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		iZeile++;
		jpaWorkingOn.add(wcbMonatsbetrachtung, new GridBagConstraints(0, iZeile,
				2, 1, 0.1, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
	...
	public String getReportname() {
		return ZeiterfassungReportFac.REPORT_MASCHINENERFOLG;
	}
	...
	public JasperPrintLP getReport(String sDrucktype) throws Throwable {
		...
		sortierung = wrbSortPersonalnummer.getText();
		...
		if (wcbNurSelektiertePerson.isSelected() == true) {
			...
			return DelegateFactory
					...
					.printMaschinenerfolg(personalIId, personalgruppeIId, wdfVon.getTimestamp(),wdfBisTemp,
							iSortierung, sortierung,
							wcbMonatsbetrachtung.isSelected()).getJasperPrintLP();
		} else {
			return DelegateFactory
					.getInstance()
					.getZeiterfassungReportDelegate()
					.printMaschinenerfolg(null, personalgruppeIId, wdfVon.getTimestamp(),wdfBisTemp,
							iSortierung, sortierung,
							wcbMonatsbetrachtung.isSelected()).getJasperPrintLP();
		}
