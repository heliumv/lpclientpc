...
private TabbedPanePartner tpPartner = null;
  ...
  public PanelPASelektion(InternalFrame internalFrame,
                          String add2TitleI,
                          Object keyI,
  ...
  public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI)
    ...
    if (key == null || (key != null && key.equals(LPMain.getLockMeForNew()))) {
      ...
      sT = getTabbedPanePartner().getPartnerDto().formatFixTitelName1Name2();
    }
    else {
      // Update.
      FLRPASelektionPK pASelektionPK = (FLRPASelektionPK) key;
      getTabbedPanePartner().setPASelektionDto(
          DelegateFactory.getInstance().getPartnerDelegate().pASelektionFindByPrimaryKey(
              new PASelektionPK(
                  pASelektionPK.getPartner_i_id(),
                  pASelektionPK.getSelektion_i_id())));
      dto2Components();

      getTabbedPanePartner().getPASelektionDto().setBIsNew(false);

      String sB = getTabbedPanePartner().getPASelektionDto().getCBemerkung();
      sT = getTabbedPanePartner().getPartnerDto().formatFixTitelName1Name2()
          + (sB == null ? "" : " | " + sB);
    }
  ...
  private void dto2Components()
      throws Throwable {
    wtfBemerkung.setText(getTabbedPanePartner().getPASelektionDto().getCBemerkung());
    SelektionDto selektionDto = DelegateFactory.getInstance().
        getPartnerServicesDelegate().selektionFindByPrimaryKey(
            getTabbedPanePartner().getPASelektionDto().getSelektionIId());
  ...
  public void eventActionSave(ActionEvent e, boolean bNeedNoSaveI)
    ...
    if (allMandatoryFieldsSetDlg()) {
      ...
      if (getTabbedPanePartner().getPASelektionDto().isBIsNew()) {
        // Create.
    	  PASelektionPK pASelektionPK = DelegateFactory.getInstance().getPartnerDelegate().
            createPASelektion(getTabbedPanePartner().getPASelektionDto());
        ...
        DelegateFactory.getInstance().getPartnerDelegate().
            updatePASelektion(getTabbedPanePartner().getPASelektionDto());
      }
  ...
  protected void eventActionDelete(ActionEvent e,
    ...
    if (!isLockedDlg()) {
      DelegateFactory.getInstance().getPartnerDelegate().removePASelektion(
          new PASelektionPK(
              getTabbedPanePartner().getPASelektionDto().getPartnerIId(),
              getTabbedPanePartner().getPASelektionDto().getSelektionIId()));
  ...
  protected void eventActionSpecial(ActionEvent e)
    ...
    if (e.getActionCommand().equals(ACTION_SPECIAL_FLR_SELEKTION)) {
      ...
      panelQueryFLRSelektionAuswahl.setSelectedId(getTabbedPanePartner().getPASelektionDto().getSelektionIId());
