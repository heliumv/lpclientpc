 ...
 */
public class PanelZahlungsvorschlag
    extends PanelBasis
{
  /**
	 * 
...
private static final String ACTION_SPECIAL_TOGGLE = "action_special_zv_toggle";
  ...
  private WrapperNumberField wnfWert = null;
  ...
  private WrapperLabel wlaBetrag = null;
  private WrapperNumberField wnfBetrag = null;
  ...
  public PanelZahlungsvorschlag(InternalFrame internalFrame, String add2TitleI,
                                Object pk, TabbedPaneZahlungsvorschlag tabbedPaneZV)
      throws Throwable {
  ...
  private void components2Dto()
      throws Throwable {
    if (zvDto != null) {
      zvDto.setNOffen(wnfBetrag.getBigDecimal());
    }
  }
  ...
  private void jbInit()
      throws Throwable {
    wlaLieferant = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr(
        "label.lieferant"));
    ...
    wlaAbteilung = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr(
        "lp.abteilung"));
    ...
    wnfWert = new WrapperNumberField();
    wnfWert.setActivatable(false);
    wlaSkonto = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr("lp.skonto"));
    wnfSkonto = new WrapperNumberField();
    wnfSkonto.setActivatable(false);

    wlaBelegdatum = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr(
        "label.belegdatum"));
    wdfBelegdatum = new WrapperDateField();
    wdfBelegdatum.setActivatable(false);
    wlaFreigabedatum = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr(
        "label.freigabedatum"));
    wdfFreigabedatum = new WrapperDateField();
    wdfFreigabedatum.setActivatable(false);

    wlaOffen = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr("label.offen"));
    wnfOffen = new WrapperNumberField();
    wnfOffen.setActivatable(false);
    wlaBetrag = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr("label.betrag"));
    ...
    wlaGesamt = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr("lp.gesamtwert"));
    wnfGesamt = new WrapperNumberField();
    wnfGesamt.setActivatable(false);
    // Mandantenwaehrung
    wlaWaehrungGesamt = new WrapperLabel(LPMain.getInstance().getTheClient().
                                         getSMandantenwaehrung());
    wlaWaehrungGesamt.setHorizontalAlignment(SwingConstants.LEFT);

    wlaLieferant.setMinimumSize(new Dimension(150,
                                              Defaults.getInstance().getControlHeight()));
    wlaLieferant.setPreferredSize(new Dimension(150,
                                                Defaults.getInstance().getControlHeight()));
    wlaWaehrung1.setMinimumSize(new Dimension(30, Defaults.getInstance().getControlHeight()));
    wlaWaehrung1.setPreferredSize(new Dimension(30,
                                                Defaults.getInstance().getControlHeight()));
    wlaWaehrung2.setMinimumSize(new Dimension(30, Defaults.getInstance().getControlHeight()));
    wlaWaehrung2.setPreferredSize(new Dimension(30,
                                                Defaults.getInstance().getControlHeight()));
    wlaSkonto.setMinimumSize(new Dimension(40, Defaults.getInstance().getControlHeight()));
    wlaSkonto.setPreferredSize(new Dimension(40, Defaults.getInstance().getControlHeight()));
    wlaSkontoProzent.setMinimumSize(new Dimension(30,
                                                  Defaults.getInstance().getControlHeight()));
    wlaSkontoProzent.setPreferredSize(new Dimension(30,
        Defaults.getInstance().getControlHeight()));
    ...
    jPanelWorkingOn = new JPanel(new MigLayout("wrap 6", "[fill,15%|fill,30%|fill,5%|fill,15%|fill,30%|fill,5%]"));
    ...
    jPanelWorkingOn.add(wtfLieferant, "span");
    
    jPanelWorkingOn.add(wtfAdresse, "skip, span");

    jPanelWorkingOn.add(wlaAbteilung);
    jPanelWorkingOn.add(wtfAbteilung, "span");
    ...
    jPanelWorkingOn.add(wlaGesamt, "skip 2, span 2");
  ...
  private void holeEingangsrechnungDto(Integer key)
      throws Throwable {
    if (key != null) {
      this.erDto = DelegateFactory.getInstance().getEingangsrechnungDelegate().
  ...
  private void dto2ComponentsER()
    ...
    if (erDto != null) {
      ...
      wtfAbteilung.setText(lieferantDto.getPartnerDto().getCName3vorname2abteilung());
      ...
      BigDecimal bdWert = erDto.getNBetragfw();
      wnfWert.setBigDecimal(bdWert);
      wnfSkonto.setBigDecimal(zvDto.getNAngewandterskontosatz());
      BigDecimal bdBezahlt = DelegateFactory.getInstance().getEingangsrechnungDelegate().
          getBezahltBetragFw(erDto.getIId(), null);
      wnfOffen.setBigDecimal(bdWert.subtract(bdBezahlt));
      wnfBetrag.setBigDecimal(zvDto.getNOffen());
    }
    else {
  ...
  protected void updateGesamtwert()
      throws Throwable {
    if (tabbedPaneZV.getZVlaufDto() != null) {
      BigDecimal bdWert = DelegateFactory.getInstance().getEingangsrechnungDelegate().
          getGesamtwertEinesZahlungsvorschlaglaufsInMandantenwaehrung(tabbedPaneZV.
          getZVlaufDto().getIId());
    ...
    if (allMandatoryFieldsSetDlg()) {
      ...
      if (zvDto != null) {
        ZahlungsvorschlagDto savedDto = DelegateFactory.getInstance().
            getEingangsrechnungDelegate().updateZahlungsvorschlag(zvDto);
        this.zvDto = savedDto;
