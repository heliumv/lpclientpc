...
public class ReportNichterfassteartikel
...
protected JPanel jpaWorkingOn = new JPanel();
  ...
  public ReportNichterfassteartikel(InternalFrameArtikel internalFrame,
    ...
    initComponents();
    if (inventurIId != null) {
      InventurDto inventurDto=DelegateFactory.getInstance().getInventurDelegate().inventurFindByPrimaryKey(
          inventurIId);
      wtfInventur.setText(inventurDto.getCBez());

      if(inventurDto.getLagerIId()!=null){
        lagerIId=inventurDto.getLagerIId();
        LagerDto lagerDto=DelegateFactory.getInstance().getLagerDelegate().lagerFindByPrimaryKey(inventurDto.getLagerIId());
  ...
  private void jbInit()
    ...
    wcbNurArtikelMitLagerstand.setText(LPMain.getInstance().getTextRespectUISPr("artikel.inventur.lagerstandgroessernull"));
  ...
  public JasperPrintLP getReport(String sDrucktype)
      throws Throwable {
    return DelegateFactory.getInstance().getInventurDelegate().printNichterfassteartikel(inventurIId,
        lagerIId,wcbNurArtikelMitLagerstand.isSelected());
  }
