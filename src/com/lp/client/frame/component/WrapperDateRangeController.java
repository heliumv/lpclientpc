...
public class WrapperDateRangeController extends JPanel implements
		ActionListener {
	/**
	 * 
	 */
	...
	public WrapperDateRangeController(WrapperDateField wdfVon,
			WrapperDateField wdfBis) {
		this.wdfVon = wdfVon;
		this.wdfBis = wdfBis;
		jbInit();
	}
	...
	private void jbInit() {
		...
		this.add(wbuDown, new GridBagConstraints(0, 0, 1, 1, 0.0, 1.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(
						0, 0, 0, 0), 0, 0));
		this.add(wbuUp, new GridBagConstraints(0, 1, 1, 1, 0.0, 1.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(
						0, 0, 0, 0), 0, 0));
	...
	public Timestamp getTimestampVon() {
		if (wdfVon.getTimestamp() != null) {
			return Helper.cutTimestamp(wdfVon.getTimestamp());
		} else {
			return null;
		}
	}
	...
	public Timestamp getTimestampBis() {
		if (wdfBis.getTimestamp() != null) {
			Calendar c=Calendar.getInstance();
			c.setTimeInMillis(wdfBis.getTimestamp().getTime());
	...
	public DatumsfilterVonBis getDatumsfilterVonBis(){
		DatumsfilterVonBis df=new DatumsfilterVonBis(wdfVon.getTimestamp(),wdfBis.getTimestamp());
		return df;
	}
