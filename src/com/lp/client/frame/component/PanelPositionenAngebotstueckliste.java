 ...
 */
public class PanelPositionenAngebotstueckliste
    extends PanelPositionenPreiseingabe
{
  /**
	 * 
	 */
...
public final static String ACTION_SPECIAL_AGSTKL_FROM_LISTE =
      "action_special_agstkl_from_liste";
   ...
   */
  public PanelPositionenAngebotstueckliste(
    ...
    super(internalFrame, add2TitleI, key, sLockMeWer,
          internalFrame.bRechtDarfPreiseSehenVerkauf, internalFrame.bRechtDarfPreiseAendernVerkauf,  iSpaltenbreite1I);
    jbInit();
    initComponents();
    initPanel();
  }
  ...
  private void jbInit()
    ...
    wbuAgstklauswahl = new WrapperButton(LPMain.getInstance().getTextRespectUISPr(
        "button.agstkl"));
    ...
    wlaBezeichnungAgstkl = new WrapperLabel(LPMain.getInstance().getTextRespectUISPr(
        "label.bezeichnung"));

    wtfBezeichnungAgstkl = new WrapperTextField();

    wcbAlternativeAgstkl = new WrapperCheckBox(
      LPMain.getInstance().getTextRespectUISPr("angb.alternative"));
    ...
    add(wbuAgstklauswahl, new GridBagConstraints(0, iYGridBagNext, 1, 1, 0.0, 0.0
    ...
    add(wtfAgstkl, new GridBagConstraints(1, iYGridBagNext, 1, 1, 0.0, 0.0
        , GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2),
        0, 0));
    add(wlaBezeichnungAgstkl, new GridBagConstraints(2, iYGridBagNext, 1, 1, 0.0, 0.0
        , GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2),
        0, 0));
    add(wtfBezeichnungAgstkl, new GridBagConstraints(3, iYGridBagNext, 5, 1, 0.0, 0.0
        , GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2),
        0, 0));
    ...
    add(wcbAlternativeAgstkl,
                           new GridBagConstraints(1, 8, 1, 1, 0.0, 0.0
                                                  , GridBagConstraints.NORTH,
                                                  GridBagConstraints.HORIZONTAL,
                                                  new Insets(10, 0, 2, 2),
                                                  0, 0));
    add(wlaKalkulatorischerWertAgstkl,
                           new GridBagConstraints(3, 8, 3, 1, 0.0, 0.0
                                                  , GridBagConstraints.NORTH,
                                                  GridBagConstraints.HORIZONTAL,
                                                  new Insets(10, 2, 2, 2),
                                                  0, 0));
    add(wnfKalkulatorischerWertAgstkl,
                           new GridBagConstraints(6, 8, 1, 1, 0.0, 0.0
                                                  , GridBagConstraints.NORTH,
                                                  GridBagConstraints.HORIZONTAL,
                                                  new Insets(10, 2, 2, 2),
                                                  0, 0));
    add(wlaWaehrungKalkulatorischerWertAgstkl,
  ...
  public void agstklDto2components()
      throws Throwable {
    wtfAgstkl.setText(agstklDto.getCNr());
    wtfBezeichnungAgstkl.setText(agstklDto.getCBez());

    BigDecimal nKalkulatorischerWert =
        DelegateFactory.getInstance().getAngebotstklDelegate().berechneKalkulatorischenAgstklwert(
            agstklDto.getIId(),
            belegwaehrungCNr);

    wnfKalkulatorischerWertAgstkl.setBigDecimal(nKalkulatorischerWert);

    // IMS 1565 Wenn eine AGSTKL neu uebernommen wird, den kalkulatorischen Wert
  ...
  public void dialogQueryAgestkl(ActionEvent e) throws Throwable {
    panelQueryFLRAgstklauswahl = AngebotstklFilterFactory.getInstance().
        createPanelFLRAgstkl(getInternalFrame(),
                             true,
                             false);
