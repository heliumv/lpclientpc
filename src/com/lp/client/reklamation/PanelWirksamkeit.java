...
public class PanelWirksamkeit extends PanelBasis {
	...
	private WirksamkeitDto wirksamkeitDto = null;

	public PanelWirksamkeit(InternalFrame internalFrame, String add2TitleI,
	...
	public void eventActionNew(EventObject eventObject, boolean bLockMeI,
		...
		wirksamkeitDto = new WirksamkeitDto();
	...
	protected void eventActionDelete(ActionEvent e,
		...
		DelegateFactory.getInstance().getReklamationDelegate()
				.removeWirksamkeit(wirksamkeitDto);
	...
	protected void components2Dto() {
		...
		wirksamkeitDto.setCBez(wtfKennung.getText());

		if (wirksamkeitDto.getWirksamkeitsprDto() == null) {
			wirksamkeitDto.setWirksamkeitsprDto(new WirksamkeitsprDto());
		}
		wirksamkeitDto.getWirksamkeitsprDto().setCBez(wtfBezeichnung.getText());

	}

	protected void dto2Components() {
		wtfKennung.setText(wirksamkeitDto.getCBez());
		if (wirksamkeitDto.getWirksamkeitsprDto() != null) {
			wtfBezeichnung.setText(wirksamkeitDto.getWirksamkeitsprDto()
					.getCBez());
		} else {
	...
	public void eventActionSave(ActionEvent e, boolean bNeedNoSaveI)
		...
		if (allMandatoryFieldsSetDlg()) {
			components2Dto();
			if (wirksamkeitDto.getIId() == null) {
				wirksamkeitDto.setIId(DelegateFactory.getInstance()
						.getReklamationDelegate()
						.createWirksamkeit(wirksamkeitDto));
				setKeyWhenDetailPanel(wirksamkeitDto.getIId());
			} else {
				DelegateFactory.getInstance().getReklamationDelegate()
						.updateWirksamkeit(wirksamkeitDto);
			}
	...
	private void jbInit() throws Throwable {
		...
		this.add(getPanelStatusbar(), new GridBagConstraints(0, 2, 1, 1, 1.0,
				...
				new Insets(0, 0, 0, 0), 0, 0));
		jpaWorkingOn.add(wlaKennung, new GridBagConstraints(0, 0, 1, 1,
				0.1, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		jpaWorkingOn.add(wtfKennung, new GridBagConstraints(1, 0, 1, 1,
	...
	protected String getLockMeWer() throws Exception {
		return HelperClient.LOCKME_WIRKSAMKEIT;
	}
	...
	public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI)
		...
		if (key == null
			...
			wirksamkeitDto = DelegateFactory.getInstance()
					.getReklamationDelegate()
